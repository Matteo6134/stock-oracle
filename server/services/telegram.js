...
export function initTelegramBot() {
  ...
  try {
    ...
    // All legacy command handlers were REMOVED 2026-06-11; these (plus free-
    // text ticker lookup) are the entire command surface.
    bot.setMyCommands([
      { command: 'recap',     description: '🔮 All current predictions in one message' },
      { command: 'portfolio', description: '💼 Open positions & P/L' },
      { command: 'pnl',       description: '💰 Profit: today, this week, all-time' },
...
function registerCommands() {
  ...
  bot.onText(/\/start/, (msg) => {
    ...
    send(cid, [
      ...
      '\uD83D\uDCB0 /pnl \u2014 profit today, this week, all-time',
      '',
      '_Tip: type any ticker (e.g. NVDA) for a full analysis._',
      '',
